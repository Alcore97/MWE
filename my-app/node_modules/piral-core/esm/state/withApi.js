...
var PortalRenderer = function (_a) {
    var id = _a.id;
    var children = useGlobalState(function (m) { return m.portals[id] || []; });
...
var ForeignComponentContainer = /** @class */ (function (_super) {
    __extends(ForeignComponentContainer, _super);
...
function wrapReactComponent(Component, stasisOptions, piral, Wrapper) {
    return function (props) { return (React.createElement(Wrapper, __assign({}, props, { piral: piral }),
        React.createElement(ErrorBoundary, __assign({}, stasisOptions, { renderProps: props }),
            React.createElement(Component, __assign({}, props, { piral: piral }))))); };
}
function wrapForeignComponent(component, stasisOptions, piral, Wrapper) {
    return function (props) {
        var destroyPortal = useActions().destroyPortal;
        var id = React.useState(function () { return (portalIdBase++).toString(26); })[0];
        var router = React.useContext(__RouterContext);
        var state = React.useContext(StateContext).state;
        var innerProps = __assign(__assign({}, props), { piral: piral });
        React.useEffect(function () { return function () { return destroyPortal(id); }; }, []);
        return (React.createElement(Wrapper, __assign({}, innerProps),
            React.createElement(ErrorBoundary, __assign({}, stasisOptions, { renderProps: props }),
...
function wrapComponent(converters, component, piral, Wrapper, stasisOptions) {
    ...
    if (typeof component === 'object' && isNotExotic(component)) {
        var result = convertComponent(converters[component.type], component);
...
export function withApi(context, component, piral, errorType, wrapperType) {
    ...
    return wrapComponent(converters, component, piral, Wrapper, {
        ...
        renderError: function (error, props) {
            return React.createElement(PiralError, __assign({ type: errorType, error: error }, props));
        },
    });
}
